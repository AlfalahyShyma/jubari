...
echo 'ppppp';    }
    ...
    public function display()
    {
        ...
        {
            $affected = DB::select('select DISTINCT users.id as u_id,users.name as u_name from users join role_user on users.id=role_user.user_id where users.is_delete != 1');
        return view('role_user_display',['data'=>$affected]);
        }
        public function display_role_user18($id)
        {
            $affected1 = DB::select('select DISTINCT roles.name as role_n from users join role_user on users.id=role_user.user_id join roles on role_user.role_id=roles.id where users.is_delete != 1 and users.id='.$id);
            echo json_encode($affected1);
        }
